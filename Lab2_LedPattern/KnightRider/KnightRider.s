...
main:
	...
	// Initialize the starting positions of LEDs on the bitmap
	ldr r4, =0xFFF00000
	movs r2, #1
...
turn_loop:
	movs r7, 0x0
	movs r5, r4
	movs r0, #8      // Initialize counter (R0) to 10
loop_start:
	movs r6, #1
	ands r6, r5, r6
	orrs r7, r6, r7
    ...
    bne loop_start   // If the counter is not zero, branch back to loop_start
	...
	cmp r2, #1                    // Compare the button status with 1 (pressed)
	beq directionr            // Controls the direction of the shifting

directionl:
	movs r6, #28
    rors r4, r4, r6
    b end_if                      //Skip the button_pressed code
...
end_if:
    ...
    bne turn_loop   //If the counter is not zero, branch back to loop_start
    movs r6, 0x1
    eors r2, r2, r6  //flip the direction
    b loop
